...
asyncUsing doFork = \action -> do
   ...
   t <- mask $ \restore ->
          doFork $ do
              labelMe "async:asyncUsing:doFork"
              try (restore action) >>= atomically . putTMVar var
...
withAsyncUsing doFork = \action inner -> do
  ...
  mask $ \restore -> do
    t <- doFork $ do
        labelMe "async:withAsyncUsing:doFork"
        try (restore action) >>= atomically . putTMVar var
        ...
        let tryAgain f = f `catch` \BlockedIndefinitelyOnMVar -> f
            ...
            stop = do
                ...
                uninterruptibleMask_ $ do
                  ...
                  when (count' > 0) $
                    void $ forkIO $ do
                      labelMe "async:concurently':throwTo"
  ...
  mask $ \restore ->
    let go = do r <- tryAll (restore action)
                case r of
                  Left _ -> go
                  _      -> return ()
    in forkIO (labelMe "async:forkRepeat" >> go)
